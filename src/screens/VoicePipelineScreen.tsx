...
type VoiceRoute = RouteProp<RootStackParamList, 'VoicePipeline'>;

const EMERGENCY_VOICE_PROMPT =
  'You are SurviLens emergency voice assistant. User may be injured or panicking. Give very short, calm, step-by-step first aid. Ask clarifying questions if needed. Never joke. Keep answers under 3 sentences when possible.';

export const VoicePipelineScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  const route = useRoute<VoiceRoute>();
  const emergencyMode = route.params?.mode === 'emergency';
  const modelService = useModelService();

  const systemPrompt = useMemo(
    () =>
      emergencyMode
        ? EMERGENCY_VOICE_PROMPT
  ...
  if (!modelService.isVoiceAgentReady) {
    return (
      <View style={styles.container}>
        <LinearGradient colors={['#059669', '#047857']} style={styles.screenHeader}>
          <TouchableOpacity onPress={() => navigation.goBack()} style={styles.headerBack}>
            <Text style={styles.headerBackText}>←</Text>
          </TouchableOpacity>
          <Text style={styles.screenHeaderTitle}>
            {emergencyMode ? 'Voice emergency' : 'Voice Agent'}
          </Text>
          <View style={{ width: 40 }} />
        </LinearGradient>
        <ModelLoaderWidget
        title={emergencyMode ? 'Voice emergency setup' : 'Voice Agent Setup Required'}
        subtitle="Download LLM + STT + TTS for hands-free help (say HELP or describe what happened)"
        icon="pipeline"
...
const styles = StyleSheet.create({
  ...
  screenHeaderTitle: { fontSize: 18, fontWeight: '700', color: '#fff' },
  container: {
    flex: 1,
